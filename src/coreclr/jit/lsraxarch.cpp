    ...
    else
    {
        ...
        switch (intrinsicId)
        {
            ...
            case NI_Vector_ToScalar:
            {
                ...
                else if (varTypeIsLong(baseType) && !m_compiler->compOpportunisticallyDependsOn(InstructionSet_SSE42))
                {
...
int LinearScan::BuildIndir(GenTreeIndir* indirTree)
{
    ...
    if (indirTree->TypeIs(TYP_SIMD12) && indirTree->OperIs(GT_STOREIND) &&
        !m_compiler->compOpportunisticallyDependsOn(InstructionSet_SSE42) && !indirTree->Data()->IsVectorZero())
    {
